...
def write_status(
    ...
    try:
        ...
        logger.info(
            f"[VERSIONADO] Status actualizado: case_id={case_id}, version={version}, status={status}"
        )
...
def validate_version_integrity(
    ...
    logger.info(
        f"[VALIDACIÓN] Iniciando validación de integridad: case_id={case_id}, version={version}"
    )
...
def cleanup_old_versions(case_id: str, keep_last: int = 3) -> int:
    ...
    versions_to_delete = [v for v in versions if v.version not in versions_to_keep]

    if not versions_to_delete:
        logger.info("[HOUSEKEEPING] No hay versiones para eliminar")
        return 0
    ...
    logger.info(
        f"[HOUSEKEEPING] Limpieza completada: " f"case_id={case_id}, eliminadas={deleted_count}"
    )
