...
class TimelineEvent(BaseModel):
    ...
    # Metadata adicional
    is_within_suspect_period: Optional[bool] = Field(None, description="¿Dentro de periodo sospechoso?")
    related_event_ids: List[str] = Field(default_factory=list, description="IDs de eventos relacionados")
...
def detect_event_type(text: str, filename: str) -> Tuple[EventType, EventCategory, EventSeverity]:
    ...
    if any(keyword in combined for keyword in ['venta', 'enajenaci', 'transmisi', 'cesión']):
        ...
        if 'participaciones' in combined or 'acciones' in combined:
            return EventType.TRANSMISION_PARTICIPACIONES, EventCategory.PATRIMONIAL, EventSeverity.HIGH
...
def build_timeline(
    db: Session,
    case_id: str,
    concurso_date: Optional[datetime] = None
) -> Timeline:
    """
    ...
    if concurso_date:
        ...
        for event in events:
            event.is_within_suspect_period = (
                suspect_period_start <= event.date <= concurso_date
            )
