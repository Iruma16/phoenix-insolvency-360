...
class AnalysisAlert(BaseModel):
    ...
    case_id: str = Field(
        ...,
        description="ID del caso al que pertenece",
        min_length=1
    )
    
    alert_type: AlertType = Field(
        ...,
        description="Tipo técnico de alerta (NO legal)"
    )
    
    description: str = Field(
        ...,
        description="Descripción técnica objetiva",
        min_length=10
    )
    
    evidence: List[AlertEvidence] = Field(
        ...,
        description="Lista de evidencia física (≥1 item)",
        min_length=1
    )
    
    created_at: datetime = Field(
        ...,
        description="Timestamp de detección de la alerta"
    )
    ...
    def validate_description_is_technical(cls, v: str) -> str:
        ...
        prohibited_legal_terms = [
            'culpable', 'inocente', 'delito', 'fraude', 'crimen',
            'culpabilidad', 'responsabilidad penal', 'condena'
        ]
