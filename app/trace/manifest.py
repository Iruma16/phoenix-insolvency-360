...
from app.trace.models import TRACE_SCHEMA_VERSION, ExecutionTrace
...
class ExecutionLimits(BaseModel):
    ...
    cannot_process_without_documents: bool = Field(
        default=True, description="El sistema NO puede responder sin documentos"
    )
    cannot_generate_without_evidence: bool = Field(
        default=True, description="El sistema NO puede generar respuestas sin evidencia"
    )
    cannot_guarantee_legal_validity: bool = Field(
        default=True, description="El sistema NO garantiza validez legal (solo técnica)"
    )
    requires_human_review: bool = Field(
        default=True, description="El sistema REQUIERE revisión humana antes de uso legal"
    )
...
class HardManifest(BaseModel):
    ...
    manifest_version: str = Field(default=MANIFEST_VERSION, description="Versión del manifest")

    trace_id: str = Field(..., description="ID del trace asociado", min_length=1)

    case_id: str = Field(..., description="ID del caso analizado", min_length=1)

    schema_versions: SchemaVersions = Field(..., description="Versiones de schemas utilizados")

    integrity_hash: str = Field(
        ..., description="Hash SHA256 del trace completo", min_length=64, max_length=64
    )

    execution_limits: ExecutionLimits = Field(
        default_factory=ExecutionLimits, description="Límites explícitos del sistema"
    )

    finops_snapshot: Optional[FinOpsSnapshot] = Field(
        None, description="Snapshot de costes (si FinOps activo)"
    )

    signed_at: datetime = Field(..., description="Timestamp de firma del manifest")

    system_version: str = Field(..., description="Versión del sistema que generó el manifest")
