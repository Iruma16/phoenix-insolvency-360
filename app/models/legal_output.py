...
from app.core.exceptions import ErrorSeverity, PhoenixException
...
class EvidenceLocation(BaseModel):
    ...
    page_start: Optional[int] = Field(
        None, description="Página de inicio (1-indexed, None solo si formato no lo permite)", ge=1
    )
    page_end: Optional[int] = Field(None, description="Página de fin (1-indexed)", ge=1)
    start_char: int = Field(
        ..., description="Offset de inicio en texto original (OBLIGATORIO)", ge=0
    )
    end_char: int = Field(..., description="Offset de fin en texto original (OBLIGATORIO)", ge=0)
...
class DocumentalEvidence(BaseModel):
    ...
    chunk_id: str = Field(..., description="ID único del chunk (trazabilidad)", min_length=1)
    document_id: str = Field(..., description="ID del documento origen", min_length=1)
    location: EvidenceLocation = Field(..., description="Ubicación física exacta en el documento")
    content: str = Field(..., description="Texto literal extraído del documento", min_length=1)
    filename: Optional[str] = Field(None, description="Nombre del archivo origen (para auditoría)")
...
class LegalFinding(BaseModel):
    ...
    statement: str = Field(
        ..., description="Afirmación objetiva, verificable y no interpretativa", min_length=10
    )
    evidence: list[DocumentalEvidence] = Field(
        ..., description="Evidencias documentales que respaldan la afirmación", min_length=1
    )
    confidence_note: Optional[str] = Field(
        None, description="Nota sobre limitaciones de la evidencia (opcional)"
    )
...
class LegalReport(BaseModel):
    ...
    case_id: str = Field(..., description="ID del caso analizado", min_length=1)
    issue_analyzed: str = Field(
        ..., description="Cuestión jurídica o consulta concreta analizada", min_length=10
    )
    findings: list[LegalFinding] = Field(
        ..., description="Lista estructurada y ordenada de hallazgos", min_length=1
    )

    # Metadatos de auditoría (OBLIGATORIOS)
    generated_at: datetime = Field(..., description="Timestamp de generación (auditoría)")
    schema_version: str = Field(
        default=LEGAL_OUTPUT_SCHEMA_VERSION, description="Versión del esquema de salida"
    )
    source_system: str = Field(default="phoenix_legal", description="Sistema que generó el reporte")

    # Metadatos adicionales (opcionales)
    total_documents_analyzed: Optional[int] = Field(
        None, description="Número total de documentos consultados", ge=0
    )
    total_chunks_used: Optional[int] = Field(
        None, description="Número total de chunks usados como evidencia", ge=0
    )
    ...
    def get_summary_stats(self) -> dict:
        ...
        unique_documents = len(set(e.document_id for f in self.findings for e in f.evidence))
        ...
        return {
            ...
            "avg_evidences_per_finding": round(total_evidences / total_findings, 2)
            if total_findings > 0
            else 0,
        }
