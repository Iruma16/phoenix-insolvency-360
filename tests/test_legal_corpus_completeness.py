...
def test_legal_corpus_minimum_chunks():
    ...
    assert count >= UMBRAL_MINIMO_CHUNKS_TRLC, \
...
def test_legal_corpus_transversal_coverage():
    ...
    assert len(failures) == 0, \
        f"Fallos en cobertura transversal:\n" + "\n".join(failures)
    
    print(f"   ✅ Todos los conceptos transversales recuperables")
...
def test_legal_corpus_no_contamination():
    ...
    assert len(contamination_found) == 0, \
        f"Contaminación detectada: {contamination_found}"
...
def test_legal_corpus_metadata_completeness():
    ...
    assert len(incomplete_chunks) == 0, \
        f"Chunks con metadatos incompletos:\n" + "\n".join(incomplete_chunks)
    
    print(f"   ✅ Metadatos completos en todos los chunks verificados")
...
def test_legal_corpus_chunk_quality():
    ...
    assert too_short / len(sample['documents']) < 0.1, \
        f"Demasiados chunks cortos: {too_short}/{len(sample['documents'])}"
    
    print(f"   ✅ Calidad de chunks adecuada")
