...
class RuleDecision(BaseModel):
    ...
    rule_id: str = Field(
        ...,
        description="ID único de la regla (ej: TRLC_ART_5_DEBER_SOLICITUD)"
    )
    ...
    article: str = Field(
        ...,
        description="Artículo legal que fundamenta (ej: TRLC Art. 5)"
    )
    
    applies: bool = Field(
        ...,
        description="¿La regla aplica al caso? (decisión DETERMINISTA)"
    )
    ...
    evidence_found: List[str] = Field(
        default_factory=list,
        description="Evidencia encontrada en el caso"
    )
    ...
    score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Score numérico de aplicabilidad [0.0-1.0]"
    )
    
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Metadata técnica adicional (NO decisiones)"
    )
...
class RuleEngineResult(BaseModel):
    ...
    evaluated_rules: List[RuleDecision] = Field(
        default_factory=list,
        description="TODAS las reglas evaluadas (triggered + discarded)"
    )
    
    triggered_rules: List[RuleDecision] = Field(
        default_factory=list,
        description="Solo reglas que applies=True (filtro de evaluated_rules)"
    )
    ...
    summary_flags: Dict[str, bool] = Field(
        default_factory=dict,
        description="Flags técnicos booleanos (NO lenguaje legal elaborado)"
    )
    
    engine_version: str = Field(
        default=RULE_ENGINE_VERSION,
        description="Versión del Rule Engine usado"
    )
    
    evaluated_at: datetime = Field(
        default_factory=datetime.utcnow,
        description="Timestamp de evaluación"
    )
    
    case_id: str = Field(
        ...,
        description="ID del caso evaluado"
    )
    
    rulebook_version: Optional[str] = Field(
        default=None,
        description="Versión del rulebook usado"
    )
    
    execution_time_ms: Optional[float] = Field(
        default=None,
        description="Tiempo de ejecución del engine"
    )
