...
class RetrievalEvidence:
    ...
    def is_sufficient(self) -> bool:
        ...
        return (
            self.valid_chunks >= MIN_CHUNKS_REQUIRED and self.avg_similarity >= MIN_AVG_SIMILARITY
        )
...
def log_evidence_decision(
    ...
    if reason_code:
        logger.error("  Status: NO_RESPONSE")
        logger.error(f"  Reason Code: {reason_code.value}")
        logger.error("  Decision: BLOCKED - Insufficient evidence")
    else:
        logger.info("  Status: EVIDENCE_VALID")
        logger.info("  Decision: ALLOWED - Sufficient evidence")
