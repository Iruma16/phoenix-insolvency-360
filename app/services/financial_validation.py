...
def validate_balance_equation(balance: BalanceData) -> Optional[ValidationIssue]:
    ...
    if deviation > max_deviation:
        ...
        deviation_percent = (deviation / max(abs(total_activo), abs(lado_derecho))) * 100 if max(abs(total_activo), abs(lado_derecho)) > 0 else 0
...
def validate_balance_pyg_coherence(
    ...
    if abs(resultado) > 1000:
        ...
        if resultado < -100000 and balance.patrimonio_neto and balance.patrimonio_neto.value > 1000000:
...
def analyze_benford_law(numbers: List[float], min_samples: int = 30) -> Optional[ValidationIssue]:
    ...
    observed_dist = {
        digit: digit_counts.get(digit, 0) / total_count
        for digit in range(1, 10)
    }
    ...
    if chi_squared > critical_value_05:
        severity = ValidationSeverity.HIGH if chi_squared > critical_value_01 else ValidationSeverity.MEDIUM
