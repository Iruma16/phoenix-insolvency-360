...
def validate_chunk_has_location(chunk: DocumentChunk) -> None:
    ...
    if chunk.start_char is None:
        raise InvalidChunkLocationError(chunk_id=chunk.chunk_id, reason="start_char es None")

    if chunk.end_char is None:
        raise InvalidChunkLocationError(chunk_id=chunk.chunk_id, reason="end_char es None")
    ...
    if not chunk.extraction_method:
        raise InvalidChunkLocationError(chunk_id=chunk.chunk_id, reason="extraction_method es None")
...
def enforce_evidence_quality(
    ...
    for source in sources:
        ...
        if not chunk_id:
            raise InvalidChunkLocationError(chunk_id="UNKNOWN", reason="Fuente sin chunk_id")

        # Validar location fields
        if source.get("start_char") is None:
            raise InvalidChunkLocationError(
                chunk_id=chunk_id, reason="start_char faltante en fuente"
            )

        if source.get("end_char") is None:
            raise InvalidChunkLocationError(chunk_id=chunk_id, reason="end_char faltante en fuente")
...
def validate_retrieval_result(
    ...
    for source in sources:
        if not source.get("chunk_id"):
            raise InvalidChunkLocationError(chunk_id="UNKNOWN", reason="Chunk sin chunk_id")

        if not source.get("document_id"):
            raise InvalidChunkLocationError(
                chunk_id=source.get("chunk_id", "UNKNOWN"), reason="Chunk sin document_id"
            )

        if not source.get("content"):
            raise InvalidChunkLocationError(
                chunk_id=source.get("chunk_id", "UNKNOWN"), reason="Chunk sin content"
            )
