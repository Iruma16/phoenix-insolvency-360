    ...
    for doc in documents:
        ...
        for idx, chunk_meta in enumerate(chunks_with_meta):
            ...
            if chunk_meta.start_char is None or chunk_meta.end_char is None:
                logger.error(
                    f"[CHUNKING] Chunk {idx} sin offsets. doc_id={doc.document_id}"
                )
                raise RuntimeError(
            ...
            if not chunk_meta.content or not chunk_meta.content.strip():
                logger.error(
                    f"[CHUNKING] Chunk {idx} vacío. doc_id={doc.document_id}"
                )
                raise RuntimeError(
                    f"Chunk vacío: doc_id={doc.document_id}, chunk_index={idx}"
                )
...
def build_document_chunks_for_single_document(
    ...
    *,
    ...
    for idx, chunk_meta in enumerate(chunks_with_meta):
        ...
        if chunk_meta.start_char < 0:
            logger.error(f"[CHUNKING] Chunk {idx} con start_char negativo. start_char={chunk_meta.start_char}")
        ...
        if chunk_meta.start_char >= chunk_meta.end_char:
            logger.error(f"[CHUNKING] Offsets inconsistentes en chunk {idx}. start={chunk_meta.start_char}, end={chunk_meta.end_char}")
            raise RuntimeError(f"Offsets inconsistentes: doc_id={doc.document_id}, chunk_index={idx}")
        
        if chunk_meta.end_char > len(text):
            logger.error(f"[CHUNKING] Offset fuera de rango en chunk {idx}. end_char={chunk_meta.end_char}, len(text)={len(text)}")
            raise RuntimeError(f"Offset fuera de rango: doc_id={doc.document_id}, chunk_index={idx}")
        ...
        # Validar extraction_method
        if chunk_meta.extraction_method == ExtractionMethod.UNKNOWN:
            logger.error(f"[CHUNKING] Chunk {idx} con extraction_method=UNKNOWN. doc_id={doc.document_id}")
            raise RuntimeError(f"extraction_method=UNKNOWN en chunk: doc_id={doc.document_id}, chunk_index={idx}")
