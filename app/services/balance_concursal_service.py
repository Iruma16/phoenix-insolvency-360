...
from app.services.credit_classifier import TRLC_RULESET_VERSION, TRLCCreditClassifier
from app.services.financial_ratios import FinancialRatiosCalculator
from app.services.insolvency_detector import INSOLVENCY_RULES_VERSION, TRLCInsolvencyDetector
...
class BalanceConcursalService:
    ...
    def analyze_balance_concursal(
        ...
        analisis_insolvencia_actual = self.insolvency_detector.detectar_insolvencia_actual(
            balance, creditos_clasificados, impagos_exigibles or []
        )
        ...
        dictamen = self._generar_dictamen(
            analisis_insolvencia_actual, analisis_insolvencia_inminente, balance, resumen
        )
    ...
    def _generar_resumen_clasificacion(
        self, creditos: list[Credit]
    ) -> dict[str, ClassificationSummary]:
        ...
        for clasificacion in CreditClassificationTRLC:
            ...
            resumen[clasificacion.value] = ClassificationSummary(
                count=len(creditos_categoria), total=sum(c.total_amount for c in creditos_categoria)
            )

        return resumen

    def _generar_dictamen(
        self, analisis_actual: dict, analisis_inminente: dict, balance: BalanceSheet, resumen: dict
    ) -> LegalOpinion:
        ...
        if analisis_actual["existe_insolvencia_actual"]:
            ...
            total_contra_masa = resumen.get(
                "contra_la_masa", ClassificationSummary(count=0, total=Decimal(0))
            ).total
            total_privilegiados = (
                resumen.get(
                    "privilegiado_especial", ClassificationSummary(count=0, total=Decimal(0))
                ).total
                + resumen.get(
                    "privilegiado_general", ClassificationSummary(count=0, total=Decimal(0))
                ).total
            )
    ...
    def persist_balance_to_db(
        self, db: Session, output: BalanceConcursalOutput, created_by: Optional[str] = None
    ) -> BalanceSituacion:
        """
        ...
        previous_balance = (
            ...
            .filter(
                ...
                BalanceSituacion.is_active.is_(True),
            )
            .first()
        )
        ...
        balance_db = BalanceSituacion(
            ...
            patrimonio_neto=str(output.balance.patrimonio_neto)
            if output.balance.patrimonio_neto
            else None,
            ...
            insolvencia_inminente=output.analisis_insolvencia["inminente"][
                "existe_insolvencia_inminente"
            ],
