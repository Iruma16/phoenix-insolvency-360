...
from app.graphs.state_validation import validate_state, migrate_legacy_state_to_schema
...
def test_nodo_invalido_falla_validacion():
    ...
    print(f"✅ Nodo inválido falló correctamente:")
...
def test_pre_validacion_detecta_entrada_invalida():
    ...
    print(f"✅ Pre-validación detectó entrada inválida durante construcción")
...
def test_flujo_validacion_completo():
    ...
    def add_timeline_node(state):
        state["timeline"] = [
            {"date": "2024-01-15", "description": "Evento", "source_doc_id": None}
        ]
        return state
...
def test_validacion_submodelos_complejos():
    ...
    """
    from app.graphs.state_schema import Document, Risk, LegalFinding
    
    state = PhoenixState(
        case_id="test_case_008",
        inputs={
            "documents": [
                Document(doc_id="d1", doc_type="balance", content="test")
            ]
        },
        facts={
            "risks": [
