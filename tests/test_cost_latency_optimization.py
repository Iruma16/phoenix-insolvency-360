...
def mock_deps():
    ...
    with patch(
        "app.agents.agent_2_prosecutor.logic.rag_answer_internal", return_value=mock_rag_result
    ) as m_rag, patch(
        "app.agents.agent_2_prosecutor.logic.query_legal_rag", return_value=mock_legal
    ) as m_legal, patch(
        "app.agents.agent_2_prosecutor.logic.get_session_factory", return_value=mock_session_maker
    ):
