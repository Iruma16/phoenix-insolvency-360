...
def test_document_inconsistency_exists():
    ...
    assert "document_inconsistency" in risk_types, \
...
def test_document_inconsistency_severity_is_low():
    ...
    assert inconsistency_risk["severity"] == "low", \
...
def test_all_core_risk_types_detected():
    ...
    for expected in expected_risk_types:
        assert expected in detected_types, \
            f"Tipo de riesgo esperado '{expected}' no fue detectado"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   ✅ document_inconsistency detectado con severity 'low'")
