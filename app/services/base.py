...
class BaseService:
    ...
    def __init__(self, db: Session, logger: Optional[StructuredLogger] = None):
        """
    ...
    def _handle_exception(
        self, error: Exception, context: str, case_id: Optional[str] = None
    ) -> PhoenixException:
        """
        ...
        if isinstance(error, PhoenixException):
            self._log_error(f"Phoenix exception in {context}", error=error, case_id=case_id)
            return error

        # Wrap en PhoenixException genérica
        self._log_error(f"Unexpected error in {context}", error=error, case_id=case_id)
