...
class AcusacionProbatoria(BaseModel):
    ...
    obligacion_legal: ObligacionLegal = Field(..., description="Ley + artículo + deber específico")

    # REQUISITO 2: Evidencia documental trazable
    evidencia_documental: list[EvidenciaDocumental] = Field(
        ..., min_items=1, description="Evidencias trazables (mínimo 1)"
    )
    ...
    severidad: Severidad = Field(..., description="Severidad evaluada (BAJA|MEDIA|ALTA|CRITICA)")

    # REQUISITO 5: Nivel de confianza CALCULADO (NO hardcoded)
    nivel_confianza: float = Field(
        ..., ge=0.0, le=1.0, description="Nivel de confianza calculado (0.0-1.0)"
    )

    # REQUISITO 6: Evidencia faltante explícita
    evidencia_faltante: list[EvidenciaFaltante] = Field(
        default_factory=list, description="Evidencias específicas que faltan (estructurado)"
    )
...
class SolicitudEvidencia(BaseModel):
    ...
    evidencia_requerida: list[str] = Field(
        ..., min_items=1, description="Lista específica de documentos/evidencias necesarias"
    )
...
class ProsecutorResult(BaseModel):
    ...
    acusaciones: list[AcusacionProbatoria] = Field(
        default_factory=list, description="Acusaciones probatorias completas"
    )

    acusaciones_bloqueadas: list[AcusacionBloqueada] = Field(
        default_factory=list, description="Acusaciones bloqueadas por evidencia insuficiente"
    )

    solicitud_evidencia: SolicitudEvidencia | None = Field(
        None, description="Solicitud explícita si TODO está bloqueado"
    )
    ...
    total_bloqueadas: int = Field(
        default=0, description="Número de acusaciones bloqueadas por evidencia insuficiente"
    )

    # ENDURECIMIENTO #6: Audit trace para replay determinista
    audit_trace_data: Optional[dict] = Field(
        None, description="Audit trace serializado para replay determinista (opcional)"
    )
