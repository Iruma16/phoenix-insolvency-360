...
def get_retention_backend(config: Optional[RetentionConfig] = None) -> RetentionBackend:
    ...
    if backend_type == "in-memory" or backend_type == "in_memory":
        ...
        print(f"[CERT] RETENTION_BACKEND_SELECTED backend=in-memory ttl={config.ttl_seconds or 'none'}")
        
        # ⚠️ WARNING para producción
        env = os.getenv("ENVIRONMENT", "dev")
        if env.lower() in ["production", "prod"]:
            print("[WARNING] Backend in-memory NO apto para producción. Configurar RETENTION_BACKEND=persistent")
        
        return backend
    
    elif backend_type == "persistent":
        # En producción, implementar con DB real
        print(f"[CERT] RETENTION_BACKEND_SELECTED backend=persistent ttl={config.ttl_seconds or 'none'}")
        raise NotImplementedError(
            "Backend persistente requiere implementación específica:\n"
