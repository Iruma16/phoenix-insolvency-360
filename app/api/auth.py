...
async def login(
    ...
    if not user_model:
        logger.warning("Intento de login fallido", action="auth_failed", email=login_data.email)
        raise HTTPException(
...
async def register(
    ...
    if not current_user.is_admin:
        logger.warning(
            "Intento de registro sin permisos", action="register_forbidden", user_id=current_user.id
        )
        raise HTTPException(
