...
def test_rag_block():
    ...
    try:
        ...
        db.commit()
        print(f"✅ 5 documentos críticos creados (sin chunks/embeddings)")
        ...
        if quality_score < LEGAL_QUALITY_SCORE_BLOCK_THRESHOLD:
            ...
            assert not result.context_text or len(result.context_text) == 0, \
                "No debería haber contexto disponible cuando está bloqueado"
            assert result.hallucination_risk == True, \
            ...
            assert any("bloqueo" in w.lower() or "calidad" in w.lower() for w in result.warnings), \
                "Debería haber warning de bloqueo por calidad"
            
            print(f"\n✅ BLOQUEO VERIFICADO:")
            print(f"   - RAG bloqueado correctamente (score: {quality_score:.1f} < {LEGAL_QUALITY_SCORE_BLOCK_THRESHOLD})")
            print(f"   - Mensaje orientado a valor legal")
            print(f"   - Hallucination risk activado")
            print(f"   - Warnings apropiados")
        else:
            print(f"\n⚠️  Caso no bloqueado (score: {quality_score:.1f} >= {LEGAL_QUALITY_SCORE_BLOCK_THRESHOLD})")
            print(f"   Pero debería tener warnings por calidad baja")
