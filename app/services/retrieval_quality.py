...
from app.core.variables import DATA, EMBEDDING_MODEL
...
def save_ground_truth_for_case(
    ...
    serializable_mapping = {
        desc: list(chunk_ids) for desc, chunk_ids in ground_truth_mapping.items()
    }
...
def load_ground_truth_for_case(case_id: str) -> list[GroundTruthQuery]:
    ...
    try:
        ...
        ground_truth_mapping = {
            desc: set(chunk_ids) for desc, chunk_ids in ground_truth_mapping.items()
        }
...
def calculate_recall_at_k(
    ...
    for gt_query in ground_truth_queries:
        ...
        if not gt_query.expected_chunk_ids:
            logger.warning(f"[RECALL@K] Saltando query sin ground truth: {gt_query.description}")
            continue
...
def print_recall_metrics(metrics: dict[str, Any]) -> None:
    ...
    for result in metrics["results_by_query"]:
        ...
        if result.recall < 0.5:
            print("   ⚠️  BAJA CALIDAD: Recall < 50%")
        elif result.recall < 0.8:
            print("   ⚠️  CALIDAD MEDIA: Recall < 80%")
        else:
            print("   ✅ BUENA CALIDAD: Recall >= 80%")
