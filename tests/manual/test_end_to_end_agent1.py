...
def test_end_to_end_agent1():
    ...
    try:
        ...
        test_doc_path.write_text("Documento de prueba para test end-to-end. Contenido relevante del caso.")
        ...
        try:
            ...
            print(f"✅ Documento guardado en BD")
        ...
        print("="*70)
        try:
            chunks_created = build_document_chunks_for_case(
                db=db,
                case_id=case_id,
                overwrite=False
            )
        ...
        try:
            with patch('openai.OpenAI') as mock_openai_class:
                ...
                embeddings_count = build_embeddings_for_case(
                    db=db,
                    case_id=case_id
                )
            ...
            with patch('openai.OpenAI') as mock_openai_class:
                ...
                with patch('app.services.document_quality.get_document_quality_summary') as mock_quality:
                    ...
                    auditor_result, auditor_fallback = run_auditor(
                        case_id=case_id,
                        question="Analiza los riesgos del caso",
                        db=db
                    )
                    print(f"✅ Auditor ejecutado")
