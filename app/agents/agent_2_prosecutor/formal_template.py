...
class DocumentoAntecedente(BaseModel):
    ...
    paginas_relevantes: list[int] = Field(
        default_factory=list, description="Páginas específicas citadas en la acusación"
    )
...
class SeccionAntecedentes(BaseModel):
    ...
    fecha_generacion: datetime = Field(
        default_factory=datetime.now, description="Fecha y hora de generación del informe"
    )

    case_id: str = Field(..., description="Identificador único del caso")

    documentos: list[DocumentoAntecedente] = Field(
        ..., min_items=1, description="Lista completa de documentos analizados (mínimo 1)"
    )
    ...
    observaciones_preliminares: str | None = Field(
        None, description="Observaciones sobre la completitud documental"
    )
...
class FuenteVerificable(BaseModel):
    ...
    extracto_literal: str = Field(
        ..., max_length=500, description="Extracto LITERAL del documento (máx. 500 caracteres)"
    )
...
class HechoProbado(BaseModel):
    ...
    fuentes: list[FuenteVerificable] = Field(
        ..., min_items=1, description="Fuentes documentales que prueban el hecho (mínimo 1)"
    )

    fecha_hecho: str | None = Field(
        None, description="Fecha en que ocurrió el hecho (si es determinable)"
    )
...
class SeccionHechosProbados(BaseModel):
    ...
    hechos: list[HechoProbado] = Field(
        ..., min_items=1, description="Lista numerada de hechos probados (mínimo 1)"
    )

    total_hechos: int = Field(..., description="Número total de hechos probados")

    resumen_cronologico: str | None = Field(
        None, description="Breve resumen cronológico de los hechos (opcional)"
    )
...
class RiesgoDetectado(BaseModel):
    ...
    titulo_riesgo: str = Field(
        ..., max_length=100, description="Título descriptivo del riesgo (máx. 100 caracteres)"
    )

    descripcion_riesgo: str = Field(..., description="Descripción detallada del riesgo legal")
    ...
    hechos_relacionados: list[int] = Field(
        ..., min_items=1, description="Números de hechos probados que sustentan este riesgo"
    )

    consecuencias_juridicas: str = Field(..., description="Consecuencias jurídicas potenciales")

    nivel_confianza: float = Field(
        ..., ge=0.0, le=1.0, description="Nivel de confianza en la evaluación (0.0-1.0)"
    )
...
class SeccionRiesgosDetectados(BaseModel):
    ...
    riesgos: list[RiesgoDetectado] = Field(
        ..., min_items=1, description="Lista de riesgos detectados (mínimo 1)"
    )
...
class PlantillaFormalAcusacion(BaseModel):
    ...
    # METADATA
    version_plantilla: str = Field(default="1.0.0", description="Versión de la plantilla formal")
    ...
    certificacion_estructural: str = Field(
        default="ESTRUCTURA_VALIDADA", description="Certificación de que la estructura es conforme"
    )
