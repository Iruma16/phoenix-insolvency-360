...
def test_decision_record_storage_explicit():
    ...
    try:
        ...
        assert (
            "[CERT] DECISION_RECORD_STORAGE" in output
        ...
        assert (
            "in-memory-dict" in output or "redis" in output or "postgres" in output
...
def test_replay_deterministic_with_scores():
    ...
    try:
        ...
        assert "[CERT] REPLAY_CONTEXT" in output, "FAIL: No aparece certificación de replay context"
        ...
        assert (
            "0.95" in output and "0.87" in output and "0.82" in output
...
def test_step_visibility():
    ...
    try:
        ...
        assert output.count("[TRACE_STEP]") >= 4, "FAIL: No aparecen todos los steps"
...
def test_no_latest_certification():
    ...
    try:
        ...
        assert (
            "[CERT] NO_LATEST_USAGE = OK" in output
...
def test_no_fallbacks_certification():
    ...
    try:
        ...
        # Validar certificación
        assert (
            "[CERT] NO_FALLBACKS = OK" in output
...
def test_no_global_state_certification():
    ...
    try:
        ...
        assert (
            "[CERT] NO_GLOBAL_STATE = OK" in output
...
def test_latest_in_versions_fails():
    ...
    """
    from app.services.tracing import DecisionRecord, get_decision_record, store_decision_record
