...
pytest.skip(
    "Legacy: este test valida PDF certificado + modelos legal_report antiguos. "
    "En la versión actual se usa `app.models.legal_output` y el flujo de PDF/trace no está en el scope del curso.",
...
from app.trace.manifest import ExecutionLimits, HardManifest, SchemaVersions
from app.trace.models import DecisionType, ExecutionTrace, TraceDecision
...
class TestBuildCertifiedPDF:
    ...
    def test_build_pdf_verifica_coherencia_case_id(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            build_certified_pdf(case_id="case-1", report=report, trace=trace, manifest=manifest)
    ...
    def test_build_pdf_con_datos_coherentes_ok(self):
        ...
        finding = LegalFinding(
            ...
            legal_basis=[LegalReference(article="164.2", description="Documentación obligatoria")],
            evidence=[evidence],
        ...
        pdf_buffer = build_certified_pdf(
            case_id=case_id, report=report, trace=trace, manifest=manifest
        )
    ...
    def test_build_pdf_es_estructuralmente_valido(self):
        ...
        assert (
            page_count >= 5
        ), f"Un informe certificado completo debe tener ≥5 páginas, encontradas {page_count}"
