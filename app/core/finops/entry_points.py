...
from .policy import LLMCallPolicy, LLMCallDecision, get_global_llm_policy
from .rag_cache import RAGCacheManager, get_global_rag_cache, compute_rag_cache_key, RAGCacheEntry
from .semantic_cache import SemanticCache, get_global_semantic_cache, compute_semantic_cache_key, SemanticCacheEntry
...
def run_embeddings(
    ...
    if provider_func is None:
        raise FinOpsBypassError(
            operation="run_embeddings",
            reason="provider_func no puede ser None"
        )
...
def run_retrieve(
    ...
    if retriever_func is None:
        raise FinOpsBypassError(
            operation="run_retrieve",
            reason="retriever_func no puede ser None"
        )
...
def run_llm_call(
    ...
    if llm_func is None:
        raise FinOpsBypassError(
            operation="run_llm_call",
            reason="llm_func no puede ser None"
        )
...
def verify_trace_coherence(
    ...
    entries_without_trace = [
        e for e in ledger.get_entries()
