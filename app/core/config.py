...
class Settings(BaseSettings):
    ...
    llm_enabled: bool = Field(
        default=True,
        env="LLM_ENABLED",
        description="Habilitar/deshabilitar LLM globalmente"
    )
    ...
    fallback_model: str = Field(
        ...
        env="FALLBACK_MODEL",
        description="Modelo LLM de fallback"
    )
    ...
    llm_timeout_seconds: int = Field(
        ...
        ge=5,
        le=120,
        description="Timeout para llamadas LLM"
    )
    ...
    llm_max_retries: int = Field(
        ...
        ge=0,
        le=5,
        description="Reintentos máximos para LLM"
    )
    ...
    rag_cache_enabled: bool = Field(
        default=True,
        env="RAG_CACHE_ENABLED",
        description="Habilitar caché de consultas RAG"
    )
    ...
    data_dir: Path = Field(
        default=Path("clients_data"),
        env="DATA_DIR",
        description="Directorio raíz de datos"
    )
    
    logs_dir: Path = Field(
        default=Path("clients_data/logs"),
        env="LOGS_DIR",
    ...
    cache_dir: Path = Field(
        default=Path("clients_data/_cache"),
        env="CACHE_DIR",
        description="Directorio de caché"
    )
    ...
    tracing_enabled: bool = Field(
        default=False,
        env="TRACING_ENABLED",
        description="Habilitar tracing distribuido"
    )
    ...
    def validate_jwt_secret(cls, v, values):
        ...
        if values.get("environment") == "production" and v == "change_this_secret_key_in_production":
            raise ValueError(
                "JWT_SECRET_KEY debe ser cambiada en producción"
            )
        return v
...
def print_config() -> None:
    ...
    print("\n" + "="*60)
    print(f"PHOENIX LEGAL - CONFIGURACIÓN")
    print("="*60)
