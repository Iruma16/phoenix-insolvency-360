...
if __name__ == "__main__":
    ...
    for finding_type, mapping in LEGAL_MAP.items():
        ...
        print(f"   ✅ {finding_type}: {len(mapping['articles'])} artículos, {len(mapping['risk_types'])} risk_types")
    ...
    for finding_type, mapping in LEGAL_MAP.items():
        for article in mapping["articles"]:
            ...
            for field in required_fields:
                assert field in article, f"{finding_type}: artículo {article.get('article', 'N/A')} debe tener campo '{field}'"
            
            # Verificar source
            assert article["source"] in valid_sources, \
                f"{finding_type}: source '{article['source']}' debe ser uno de {valid_sources}"
            
            # Verificar boe_url
            assert article["boe_url"].startswith("http"), \
                f"{finding_type}: boe_url debe empezar por 'http'"
            
            # Verificar last_verified (formato YYYY-MM-DD)
            assert date_pattern.match(article["last_verified"]), \
            ...
            try:
                ...
                raise AssertionError(f"{finding_type}: last_verified '{article['last_verified']}' no es una fecha válida")
            
            # Guardar para detección de duplicados
            article_key = (article["law"], article["article"])
            all_articles.append(article_key)
            
            print(f"   ✅ {finding_type} → {article['article']} (verificado: {article['last_verified']})")
    
    print("\n4. Verificando que no hay artículos duplicados...")
    unique_articles = set(all_articles)
    assert len(all_articles) == len(unique_articles), \
    ...
    print(f"   ✅ legal_basis incluye metadatos de verificación")
    ...
    print(f"   ✅ overall_risk sigue siendo 'high' (sin regresión)")
    ...
    for finding_type, mapping in LEGAL_MAP.items():
        for article in mapping["articles"]:
            try:
                ...
                pytest.fail(f"{finding_type}: last_verified '{article['last_verified']}' no es una fecha válida")
...
def test_all_findings_mapped():
    ...
    expected_findings = {"accounting_irregularities", "delay_filing", "document_inconsistency", "documentation_gap"}
    actual_findings = set(LEGAL_MAP.keys())
    assert expected_findings.issubset(actual_findings), \
