...
def upload_file(case_id, file_path):
    ...
    with open(path, "rb") as f:
        ...
        response = requests.post(
            f"{API_URL}/api/cases/{case_id}/documents",
            files=files
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("\nEjemplo:")
        print("  python upload_simple.py 140a476d-0325-43e7-93bd-616635997907 data/casos_prueba/RETAIL_DEMO_SL/01_Balance_Situacion_2023.pdf")
        print("\n" + "="*60)
        list_cases()
        sys.exit(1)
