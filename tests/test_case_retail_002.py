...
def test_case_retail_002_has_delay_filing_medium():
    ...
    assert delay_filing_risk["severity"] == "medium", \
...
def test_case_retail_002_has_document_inconsistency_medium():
    ...
    risks = result["risks"]
    inconsistency_risk = next((r for r in risks if r["risk_type"] == "document_inconsistency"), None)
    
    assert inconsistency_risk is not None, "No se encontró riesgo document_inconsistency"
    assert inconsistency_risk["severity"] == "medium", \
...
if __name__ == "__main__":
    ...
    try:
        ...
        assert delay_filing and delay_filing["severity"] == "medium", \
            f"Expected delay_filing 'medium', got '{delay_filing['severity'] if delay_filing else 'not found'}'"
        print("   ✅ delay_filing es 'medium'")
        
        inconsistency = next((r for r in result["risks"] if r["risk_type"] == "document_inconsistency"), None)
        assert inconsistency and inconsistency["severity"] == "medium", \
