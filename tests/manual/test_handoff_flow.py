...
def test_handoff_flow():
    ...
    with patch("app.agents.agent_1_auditor.runner.query_case_rag") as mock_rag:
        ...
        print(
            f"✅ Agente 1 completado: summary ({len(auditor_result.summary)} chars), fallback={auditor_fallback}"
        )
        ...
        with patch(
            "app.agents.agent_2_prosecutor.runner.ejecutar_analisis_prosecutor"
        ) as mock_prosecutor:
