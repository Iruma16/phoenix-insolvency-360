...
class TraceDecision(BaseModel):
    ...
    step_name: str = Field(..., description="Nombre determinista del paso", min_length=1)
    decision_type: DecisionType = Field(..., description="Tipo de decisión")
    description: str = Field(..., description="Descripción objetiva de la decisión", min_length=10)
    evidence_ids: list[str] = Field(
        default_factory=list, description="IDs de evidencia implicados en esta decisión"
    )
    validation_applied: Optional[str] = Field(None, description="Validación aplicada (si aplica)")
    timestamp: datetime = Field(
        ..., description="Timestamp de la decisión (obligatorio para ordenamiento)"
    )
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Metadata adicional de la decisión"
    )
...
class TraceError(BaseModel):
    ...
    recovered: bool = Field(
        default=False, description="Si el error fue recuperado o abortó la ejecución"
    )
...
class ExecutionTrace(BaseModel):
    ...
    trace_id: str = Field(..., description="ID determinista del trace", min_length=1)
    case_id: str = Field(..., description="ID del caso analizado", min_length=1)
    execution_timestamp: datetime = Field(..., description="Timestamp de inicio de ejecución")
    ...
    chunk_ids: list[str] = Field(
        default_factory=list, description="IDs de chunks utilizados como evidencia"
    )
    document_ids: list[str] = Field(
        default_factory=list, description="IDs de documentos consultados"
    )

    # Salida
    legal_report_hash: Optional[str] = Field(
        None, description="Hash SHA256 del reporte legal generado"
    )

    # Decisiones (ORDENADAS)
    decisions: list[TraceDecision] = Field(
        ..., description="Lista ordenada de decisiones técnicas", min_length=1
    )

    # Errores (si existen)
    errors: list[TraceError] = Field(
        default_factory=list, description="Errores ocurridos durante la ejecución"
    )
    ...
    system_version: str = Field(..., description="Versión del sistema que generó el trace")
    trace_schema_version: str = Field(
        default=TRACE_SCHEMA_VERSION, description="Versión del esquema de trace"
    )

    # Timestamp de finalización
    completed_at: Optional[datetime] = Field(
        None, description="Timestamp de finalización (None si abortó)"
    )
