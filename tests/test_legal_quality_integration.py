...
def test_quality_summary_with_critical_docs():
    ...
    try:
        ...
        assert summary.get('critical_documents_missing', 0) > 0, "Debería detectar documentos críticos faltantes"
...
def test_rag_block_on_low_quality(case_id: str):
    ...
    try:
        ...
        if quality_score < LEGAL_QUALITY_SCORE_BLOCK_THRESHOLD:
            ...
            assert result.hallucination_risk == True, "Debería tener hallucination_risk=True cuando está bloqueado"
    ...
    try:
        ...
        if summary.get('legal_risks'):
            ...
            for risk in summary['legal_risks']:
                ...
                assert any(keyword in risk.lower() for keyword in [
                    "legal", "documento", "crítico", "riesgo", "información"
