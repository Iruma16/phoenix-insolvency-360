...
class NoChunksFoundError(RAGEvidenceError):
    ...
    def __init__(self, case_id: str, **kwargs):
        super().__init__(
            reason="No hay chunks disponibles en el vectorstore", case_id=case_id, **kwargs
        )
...
class InvalidChunkLocationError(RAGEvidenceError):
    ...
    def __init__(self, chunk_id: str, reason: str, **kwargs):
        details = {"chunk_id": chunk_id, "validation_failed": reason}

        super().__init__(reason=f"Chunk {chunk_id[:16]}... sin location válida: {reason}", **kwargs)
