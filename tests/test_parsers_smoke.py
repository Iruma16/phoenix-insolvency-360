...
def test_accounting_parser_smoke():
    ...
    from app.services.accounting_parser import identify_statement_type, parse_financial_statement_from_text
...
def test_feature_flags_existence():
    ...
    print(f"✅ Feature flags: LLM={settings.enable_llm_extraction}, Vision={settings.enable_gpt_vision}")
...
def test_invoice_data_consistency():
    ...
    if invoice.subtotal and invoice.tax_amount:
        ...
        assert abs(invoice.total_amount - expected_total) < Decimal('0.02'), \
...
def test_accounting_ratios_validity():
    ...
    if stmt and stmt.balance:
        ...
        if balance.total_activo and balance.total_pasivo and balance.patrimonio_neto:
            ...
            assert abs(balance.total_activo - suma) < 1, \
...
def test_document_classifier():
    ...
    print(f"✅ DocumentClassifier funcionando correctamente")
