...
def ingest_ley_completa(overwrite=True):
    ...
    if len(valid_chunks) < 100:
        raise ValueError(f"Solo {len(valid_chunks)} chunks generados. Esperado >100 para texto completo.")
    ...
    collection = chroma_client.get_or_create_collection(
        name="chunks",
        metadata={"type": "legal", "source": "BOE"}
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"   python scripts/validate_legal_corpus.py")
