    ...
    for match in matches:
        ...
        try:
            ...
            if ',' in num_str and '.' in num_str:
                ...
                parts = num_str.split(',')
                value_str = num_str.replace(',', '.') if len(parts[-1]) == 2 else num_str.replace(',', '')
            else:
...
def create_evidence_from_line(
    ...
    return Evidence(
        ...
        excerpt=excerpt,
        extraction_method=chunk.extraction_method if chunk and chunk.extraction_method else "line_search",
...
def parse_balance_from_chunks(
    ...
    if case and hasattr(case, 'documents') and case.documents:
        ...
        balance_docs = [
            ...
            if 'balance' in (d.doc_type or '').lower() or  # ✅ doc_type, no document_type
               'balance' in (d.filename or '').lower()
        ]
    ...
    for line in lines:
        ...
        if 'activo' in line_lower and 'corriente' in line_lower and not balance.activo_corriente:
            ...
            if value is not None and value >= 0:
                ...
                conf = ConfidenceLevel.HIGH if chunk and 'excel' in str(chunk.extraction_method) else ConfidenceLevel.MEDIUM
        ...
        # Activo Total
        if (('total' in line_lower and 'activo' in line_lower) or 
            ('activo' in line_lower and 'total' in line_lower)) and not balance.activo_total:
            kw_pos = line_lower.find('activo')
            value = extract_number_from_line(line, kw_pos)
            if value is not None and value >= 0:
                chunk = find_chunk_at_offset(chunks, cumulative_offset)
        ...
        # Pasivo Total
        if (('total' in line_lower and 'pasivo' in line_lower) or 
            ('pasivo' in line_lower and 'total' in line_lower)) and not balance.pasivo_total:
            kw_pos = line_lower.find('pasivo')
            value = extract_number_from_line(line, kw_pos)
            if value is not None and value >= 0:
                chunk = find_chunk_at_offset(chunks, cumulative_offset)
    ...
    fields_high = sum([
        balance.activo_corriente and balance.activo_corriente.confidence == ConfidenceLevel.HIGH,
        balance.pasivo_corriente and balance.pasivo_corriente.confidence == ConfidenceLevel.HIGH,
...
def parse_pyg_from_chunks(
    ...
    if case and hasattr(case, 'documents') and case.documents:
        ...
        for keyword in ['pyg', 'perdidas', 'ganancias', 'p&g', 'resultado']:
            matching_docs = [
                d for d in case.documents
                if keyword in (d.doc_type or '').lower() or  # ✅ doc_type, no document_type
                   keyword in (d.filename or '').lower()
            ]
            if matching_docs:
                doc = matching_docs[0]
                break
