...
class LegalExplanationInput(BaseModel):
    ...
    case_id: str = Field(
        ...,
        description="ID del caso (para trazabilidad)"
    )
    
    rules_triggered: List[RuleDecision] = Field(
        default_factory=list,
        description="Reglas que YA fueron evaluadas como applies=True"
    )
    
    risks_detected: List[str] = Field(
        default_factory=list,
        description="Lista de riesgos detectados (risk_type strings)"
    )
    
    missing_evidence: List[str] = Field(
        default_factory=list,
        description="Evidencia faltante (para explicar limitaciones)"
    )
    
    tone: Literal["neutral", "technical", "client_friendly"] = Field(
        default="neutral",
        description="Tono del texto generado"
    )
    
    disclaimer_required: bool = Field(
        default=True,
        description="¿Incluir disclaimer legal automático?"
    )
    ...
    max_tokens: int = Field(
        ...
        ge=50,
        le=2000,
        description="Límite de tokens para la explicación"
    )
...
class LegalExplanationOutput(BaseModel):
    ...
    case_id: str = Field(
        ...,
        description="ID del caso explicado"
    )
    
    explanation: str = Field(
        ...,
        description="Explicación en lenguaje natural de las decisiones"
    )
    
    disclaimer: Optional[str] = Field(
        default=None,
        description="Disclaimer legal (si disclaimer_required=True)"
    )
    
    generated_at: str = Field(
        ...,
        description="Timestamp de generación (ISO format)"
    )
    
    tokens_used: Optional[int] = Field(
        default=None,
        description="Tokens consumidos por el LLM"
    )
