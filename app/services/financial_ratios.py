...
class FinancialRatiosCalculator:
    ...
    def calculate_ratio_liquidez(self, balance: BalanceSheet) -> RatioResult:
        ...
        if ratio < 1:
            interp = f"CRÍTICO: Activo corriente insuficiente para cubrir pasivo corriente ({ratio:.2f})"
        elif ratio < 1.5:
    ...
    def calculate_ratio_solvencia(self, balance: BalanceSheet) -> RatioResult:
        ...
        if balance.total_pasivo <= 0:
            return RatioResult(
                value=None,
                status="NO_APLICABLE",
                reason="Total pasivo es cero o negativo"
            )
    ...
    def calculate_ratio_endeudamiento(self, balance: BalanceSheet) -> RatioResult:
        ...
        if balance.total_activo <= 0:
            return RatioResult(
                value=None,
                status="NO_APLICABLE",
                reason="Total activo es cero o negativo"
            )
    ...
    def calculate_ratio_autonomia(self, balance: BalanceSheet) -> RatioResult:
        ...
        return RatioResult(
            value=ratio,
            status="CALCULADO",
            interpretation=f"Autonomía financiera: {ratio:.2%}"
        )
    ...
    def calculate_roa(self, balance: BalanceSheet, pyg: IncomeStatement) -> RatioResult:
        ...
        return RatioResult(
            value=roa,
            status="CALCULADO",
            interpretation=f"Rentabilidad económica: {roa:.2f}%"
        )
