...
class TRLCInsolvencyDetector:
    ...
    def detectar_insolvencia_actual(
        ...
        return {
            ...
            "razonamiento_juridico": self._generar_razonamiento_actual(indicadores, insolvencia_actual),
    ...
    def detectar_insolvencia_inminente(
        ...
        insolvencia_inminente = len(indicadores) >= 2 or any(i["gravedad"] == "ALTA" for i in indicadores)
        
        return {
            "existe_insolvencia_inminente": insolvencia_inminente,
            "nivel_gravedad": nivel_gravedad,
            "indicadores": indicadores,
            "razonamiento_juridico": self._generar_razonamiento_inminente(indicadores, insolvencia_inminente),
    ...
    def _generar_razonamiento_actual(self, indicadores, insolvencia_actual):
        ...
        if insolvencia_actual:
            ...
            return (
                ...
                + ("El patrimonio neto negativo agrava la situación." if any(i["tipo"] == "PATRIMONIO_NETO_NEGATIVO" for i in indicadores) else "")
            )
        else:
