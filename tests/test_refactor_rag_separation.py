...
def test_rag_retrieves_context_only():
    ...
    try:
        ...
        assert hasattr(result, "context_text"), "RAGInternalResult debe tener campo 'context_text'"
        assert not hasattr(
            result, "answer"
        ), "RAGInternalResult NO debe tener campo 'answer' (fue eliminado)"
        assert result.context_text is not None, "context_text no debe ser None"
        assert len(result.context_text) > 0, "Debe haber contexto recuperado"
        assert len(result.sources) > 0, "Debe haber al menos una fuente"
