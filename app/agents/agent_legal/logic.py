...
def _filter_legal_articles(
    legal_articles: list[str], allowed_articles: set[str], legal_context: str
    ...
    for article_ref in legal_articles:
        ...
        if normalized and normalized in allowed_articles:
            ...
            logger.warning(f"Artículo descartado por no estar en contexto legal: {article_ref}")
...
def legal_agent_logic(
    ...
    auditor_context = (
        "\n".join(auditor_context_parts)
        if auditor_context_parts
        else "No hay contexto previo del Auditor disponible."
    )
    ...
    has_legal_context = bool(
        legal_context
        and legal_context.strip()
        and legal_context != "No hay base legal disponible en el sistema."
    )
    ...
    prompt = LEGAL_AGENT_PROMPT.format(
        ...
        if legal_context
        else "No hay base legal disponible en el sistema.",
    )
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Eres un experto en Ley Concursal española. Responde siempre con JSON válido.",
                },
        ...
        if not has_legal_context:
            ...
            result_data["missing_data"].append(
                "Base legal insuficiente: no hay contexto legal disponible."
            )
        ...
        for risk_data in result_data.get("legal_risks", []):
            ...
            valid_articles, discarded_articles = _filter_legal_articles(
                original_articles, allowed_articles, legal_context
            )
        ...
        valid_basis, discarded_basis = _filter_legal_articles(
            original_basis, allowed_articles, legal_context
        )
