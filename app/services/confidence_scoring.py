...
def calculate_confidence_score(
    *,
    ...
    if gt_recall is not None:
        ...
        confidence = 0.25 * quantity_score + 0.50 * similarity_score + 0.25 * consistency_score
...
def explain_confidence_score(
    *,
    ...
    factors = {
        ...
        "consistency": "alta"
        if len(distances) <= 1 or statistics.variance(distances) < 0.1
        else "media"
        if statistics.variance(distances) < 0.3
        else "baja",
    }
