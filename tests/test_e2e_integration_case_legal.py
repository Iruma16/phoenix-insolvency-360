...
def test_both_rags_return_results():
    ...
    with get_session() as db:
        ...
        case_results = query_case_rag(db=db, case_id=TEST_CASE_ID, query=case_query)
        ...
        legal_results = query_legal_rag(
            query=legal_query, top_k=5, include_ley=True, include_jurisprudencia=False
        )
        ...
        if len(legal_results) == 0:
            pytest.skip(
                "El vectorstore legal está vacío (requiere ingesta legal). RAG de casos funciona correctamente."
            )
...
def test_no_contamination_legal_to_case():
    ...
    with get_session() as db:
        ...
        assert any(
            keyword in case_lower for keyword in ["préstamo", "prestamo", "contrato"]
...
def test_no_contamination_case_to_legal():
    ...
    if len(legal_results) == 0:
        pytest.skip(
            "El vectorstore legal está vacío (requiere ingesta legal). Aislamiento verificado estructuralmente."
        )
    ...
    for result in legal_results:
        ...
        for data in case_specific_data:
            assert data not in text_lower, f"El RAG legal no debe contener datos del caso: '{data}'"
...
def test_vectorstore_isolation():
    ...
    assert (
        case_vectorstore_path != legal_vectorstore_path
    ...
    assert not case_vectorstore_path.is_relative_to(
        legal_vectorstore_path
    ), "El vectorstore del caso no debe estar dentro del vectorstore legal"

    assert not legal_vectorstore_path.is_relative_to(
        case_vectorstore_path
...
def test_case_specific_query_vs_legal_query():
    ...
    with get_session() as db:
        ...
        case_results = query_case_rag(
            db=db, case_id=TEST_CASE_ID, query=f"{query_term} del préstamo"
        )
        ...
        assert (
            "local" in case_lower
            or "madrid" in case_lower
            or "comercial" in case_lower
            or "préstamo" in case_lower
...
def test_concurrent_rag_access():
    ...
    with get_session() as db:
        ...
        legal_result_1 = query_legal_rag(
            "concurso voluntario", top_k=3, include_jurisprudencia=False
        )
        case_result_2 = query_case_rag(db, TEST_CASE_ID, "garantía")
        legal_result_2 = query_legal_rag(
            "administrador concursal", top_k=3, include_jurisprudencia=False
        )

        # Verificar que el RAG de casos funciona
        if (
            not case_result_1
            or not case_result_2
            or len(case_result_1) == 0
            or len(case_result_2) == 0
        ):
            pytest.skip("El RAG de casos no devuelve resultados (puede requerir re-ingesta)")
        ...
        assert (
            case_result_1 != case_result_2
        ), "Consultas diferentes deben dar resultados diferentes"
        assert (
            legal_result_1 != legal_result_2
        ), "Consultas legales diferentes deben dar resultados diferentes"
