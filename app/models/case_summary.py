...
class CaseSummary(BaseModel):
    ...
    case_id: str = Field(
        ..., description="Identificador único del caso (generado por core)", min_length=1
    )

    name: str = Field(..., description="Nombre del caso", min_length=1)

    created_at: datetime = Field(..., description="Timestamp de creación del caso")

    documents_count: int = Field(
        default=0, description="Número de documentos ingresados en el caso", ge=0
    )

    last_execution_at: Optional[datetime] = Field(
        None, description="Timestamp de última ejecución de análisis (si existe)"
    )
    ...
    client_ref: Optional[str] = Field(None, description="Referencia del cliente (opcional)")
...
class CreateCaseRequest(BaseModel):
    ...
    name: str = Field(..., description="Nombre del caso", min_length=1, max_length=255)

    client_ref: Optional[str] = Field(
        None, description="Referencia del cliente (opcional)", max_length=255
    )
