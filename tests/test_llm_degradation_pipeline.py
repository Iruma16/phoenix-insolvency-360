...
def test_pipeline_sin_llm_output_valido(mock_enabled):
    ...
    llm_result = execute_llm(
        task_name="prosecutor_analysis", prompt_system="Analyze", prompt_user="Case data"
    )
    ...
    explanation = generate_degraded_explanation(
        task_name="prosecutor_analysis", rule_engine_result=rule_result, reason="LLM disabled"
    )
...
def test_pipeline_con_timeout_output_valido(mock_call, mock_enabled):
    ...
    print("   LLM timeout detectado y degradado")
...
def test_reporte_siempre_se_genera(mock_enabled):
    ...
    llm_result = execute_llm(
        task_name="generate_report", prompt_system="System", prompt_user="User"
    )

    # Generar reporte (sin LLM)
    explanation = generate_degraded_explanation(
        task_name="generate_report", rule_engine_result=rule_result, reason="LLM disabled"
    )
