...
from app.reports.pdf_report import build_report_payload, render_pdf, generate_report_for_case
...
SYNTHETIC_ANALYSIS_RESULT = {
    ...
    "risks": [
        ...
        },
        {
            ...
            "evidence": [
                {"summary": "Discrepancias en balance", "doc_id": "doc_002"}
            ]
        }
    ],
    "legal_findings": [
...
def test_build_report_payload():
    ...
    print(f"   ✅ Payload construido correctamente")
...
def test_render_pdf_creates_file(cleanup_test_reports):
    ...
    print(f"   ✅ Archivo es un PDF válido (header correcto)")
...
def test_pdf_contains_case_id(cleanup_test_reports):
    ...
    print(f"   ✅ PDF generado correctamente (contenido comprimido por ReportLab)")
...
def test_report_with_minimal_data(cleanup_test_reports):
    ...
    print(f"   ✅ PDF generado correctamente con datos mínimos")
...
def test_report_payload_evidence_marking():
    ...
    finding_without_evidence = next(
        (f for f in payload['findings'] if f['type'] == 'missing_evidence_test'),
        None
    )
    
    assert finding_without_evidence is not None, "Finding de prueba no encontrado"
    assert finding_without_evidence['has_sufficient_evidence'] is False, \
        "El finding sin evidencia debe estar marcado como has_sufficient_evidence=False"
    
    print(f"   ✅ Findings sin evidencia correctamente marcados")
