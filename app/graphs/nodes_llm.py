...
def auditor_llm_node(state: AuditState) -> dict[str, Any]:
    ...
    if result.get("llm_enabled"):
        ...
        print("   ⚠️  Auditor LLM deshabilitado (sin API key)")

    # Actualizar estado
    return {"auditor_llm": result}
...
def prosecutor_llm_node(state: AuditState) -> dict[str, Any]:
    ...
    # Nota: `documents` no es necesario en este nodo (evita lint F841).
    ...
    try:
        ...
        if queries:
            legal_results = query_legal_rag(
                query=" ".join(queries), top_k=5, include_ley=True, include_jurisprudencia=False
            )
    ...
    result = agent.analyze(
        case_id=case_id, risks=risks, legal_findings=legal_findings, legal_rag_context=legal_context
    )
    ...
    if result.get("llm_enabled"):
        ...
        print("   ⚠️  Prosecutor LLM deshabilitado (sin API key)")

    # Actualizar estado
    return {"prosecutor_llm": result}
