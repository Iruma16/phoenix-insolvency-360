...
def test_no_pii_in_trace_logs():
    ...
    session = TracingSession(component="RAG", case_id="test_case_security_001")
...
def test_case_id_isolation():
    ...
    retrieve_path = (
        Path(__file__).resolve().parent.parent / "app" / "rag" / "case_rag" / "retrieve.py"
    )
...
def test_retention_policy_explicit():
    ...
    print(
        f"[CERT] RETENTION_POLICY backend={_STORAGE_BACKEND} retention={_STORAGE_RETENTION_POLICY}"
    )
...
def test_delete_by_case_id_capability():
    ...
    def delete_by_case_id(case_id_to_delete):
        to_delete = [
            rid
            for rid, rec in _DECISION_RECORDS_STORAGE.items()
            if rec.case_id == case_id_to_delete
        ]
