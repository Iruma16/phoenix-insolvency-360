...
from app.core.llm_config import FALLBACK_MODEL, PRIMARY_MODEL, is_llm_enabled
...
class LLMExecutionResult(BaseModel):
    ...
    success: bool = Field(..., description="¿La ejecución fue exitosa?")

    output_text: Optional[str] = Field(
        default=None, description="Texto generado por el LLM (None si falló)"
    )

    error_type: Optional[
        Literal["disabled", "no_api_key", "timeout", "api_error", "validation_error", "unknown"]
    ] = Field(default=None, description="Tipo de error (si success=False)")

    error_message: Optional[str] = Field(default=None, description="Mensaje de error detallado")

    retries_used: int = Field(default=0, description="Número de reintentos realizados")

    model_used: Optional[str] = Field(default=None, description="Modelo usado (primary o fallback)")

    degraded: bool = Field(default=False, description="¿Se activó modo degradado?")

    latency_ms: Optional[float] = Field(default=None, description="Latencia total de ejecución")

    task_name: str = Field(..., description="Nombre de la tarea ejecutada")

    executed_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp de ejecución"
    )
...
def execute_llm(
    *,
    ...
    # ════════════════════════════════════════════════════════════════
    print(
        f"[LLM_EXECUTOR] task={task_name} primary_failed={primary_model} "
    ...
    # ════════════════════════════════════════════════════════════════
    print(f"[LLM_EXECUTOR] task={task_name} result=DEGRADED " f"reason=all_models_failed")
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Éxito!
            print(
                f"[LLM_EXECUTOR] task={task_name} model={model} "
...
def _call_llm_api(
    *, prompt_system: str, prompt_user: str, model: str, timeout_seconds: int, max_tokens: int
) -> str:
    """
...
def generate_degraded_explanation(*, task_name: str, rule_engine_result, reason: str) -> str:
    """
