...
from .schema import AcusacionProbatoria, AcusacionBloqueada, ProsecutorResult
...
class AuditTrace:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AuditTrace":
        ...
        rag_snapshots = [
            RAGSnapshot(**snap) for snap in data.get("rag_snapshots", [])
        ]
        
        prosecutor_snapshot = ProsecutorSnapshot(**data["prosecutor_snapshot"])
        
        invariants_checked = [
            GateCheck(**inv) for inv in data.get("invariants_checked", [])
        ]
        ...
        llm_call_decisions = [
            LLMCallDecisionSnapshot(**dec)
            for dec in data.get("llm_call_decisions", [])
        ]
