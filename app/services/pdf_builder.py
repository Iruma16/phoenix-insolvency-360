...
from app.trace.manifest import ExecutionLimits, HardManifest
...
def _build_cover_page(
    case_id: str, report_id: str, trace_id: str, manifest: HardManifest, styles: dict
) -> list:
    """
    ...
    title = Paragraph("<b>INFORME LEGAL CERTIFICADO</b>", styles["Title"])
...
def _build_legal_disclaimer(manifest: HardManifest, report: LegalReport, styles: dict) -> list:
    """
...
def _build_legal_findings(report: LegalReport, styles: dict) -> list:
    """
    ...
    if report.findings:
        elements.append(
            Paragraph(
                f"<b>Hallazgos Identificados ({len(report.findings)}):</b>", styles["Heading2"]
            )
        )
        elements.append(Spacer(1, 0.5 * cm))
        ...
        for idx, finding in enumerate(report.findings, 1):
            # Título del hallazgo
            elements.append(
                Paragraph(f"<b>Hallazgo {idx}: {finding.title}</b>", styles["Heading3"])
            )
            elements.append(Spacer(1, 0.3 * cm))
            ...
            for legal_ref in finding.legal_basis:
                legal_text = (
                    f"• {legal_ref.law_name}, Art. {legal_ref.article}: {legal_ref.description}"
                )
            ...
            if len(finding.evidence) > 3:
                more_text = (
                    f"  ... y {len(finding.evidence) - 3} elementos adicionales de evidencia."
                )
...
def _build_traceability_summary(trace: ExecutionTrace, styles: dict) -> list:
    """
    ...
    if trace.completed_at:
        exec_info.append(
            f"<b>Finalización:</b> {trace.completed_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
    ...
    if trace.decisions:
        elements.append(
            Paragraph("<b>Timeline de Decisiones (primeras 10):</b>", styles["Heading3"])
        )
        elements.append(Spacer(1, 0.3 * cm))
...
def _build_technical_certificate(manifest: HardManifest, styles: dict) -> list:
    """
    ...
    hash_style = ParagraphStyle(
        "MonoHash", parent=styles["Normal"], fontName="Courier", fontSize=8, wordWrap="CJK"
    )
...
def build_certified_pdf(
    case_id: str, report: LegalReport, trace: ExecutionTrace, manifest: HardManifest
) -> BytesIO:
    """
    ...
    if manifest.case_id != case_id:
        raise ValueError(
            f"Incoherencia: manifest.case_id ({manifest.case_id}) != case_id ({case_id})"
        )
    ...
    styles.add(
        ParagraphStyle(name="Justify", parent=styles["Normal"], alignment=TA_JUSTIFY, fontSize=10)
    )
    ...
    # 1. Portada
    elements.extend(_build_cover_page(case_id, report.report_id, trace.trace_id, manifest, styles))
