...
def validate_state(
    state: Union[Dict[str, Any], PhoenixState],
    *,
    stage: str
) -> PhoenixState:
    """
    ...
    try:
        ...
        print(f"[STATE_VALIDATION] stage={stage} result=FAILED errors={len(e.errors())}", file=sys.stderr)
        for err in e.errors():
...
def migrate_legacy_state_to_schema(legacy_state: Dict[str, Any]) -> PhoenixState:
    ...
    migrated = {
        ...
        "timeline": {
            "events": legacy_state.get("timeline", [])
        },
        "facts": {
            "risks": legacy_state.get("risks", []),
        ...
        "legal_rules": {
            "findings": legacy_state.get("rule_based_findings", [])
        },
        "agents": {
...
def extract_legacy_state_from_schema(phoenix_state: PhoenixState) -> Dict[str, Any]:
    ...
    return {
        ...
        "auditor_llm": phoenix_state.agents.auditor_llm.model_dump() if phoenix_state.agents.auditor_llm else None,
        "prosecutor_llm": phoenix_state.agents.prosecutor_llm.model_dump() if phoenix_state.agents.prosecutor_llm else None,
        "rule_based_findings": [finding.model_dump() for finding in phoenix_state.legal_rules.findings],
