...
def test_courtpack_export_zip_contains_manifest_and_doc0(tmp_path: Path):
    ...
    # Seed: state + audit
    (doc0_dir / "state.json").write_text(
        json.dumps({"fields": {"a": "b"}, "updated_at": "t"}, indent=2), encoding="utf-8"
    )
    (doc0_dir / "audit.ndjson").write_text(
        '{"ts":"t","actor":"ui","action":"field_update","field":"a","old":null,"new":"b"}\n',
        encoding="utf-8",
    )
