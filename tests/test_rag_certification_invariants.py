...
def test_llm_not_called_on_block():
    ...
    try:
        ...
        assert (
            "[CERT] LLM_CALL_START" not in output
...
def test_llm_called_only_when_policy_passes():
    ...
    try:
        ...
        if policy_complies and len(mock_sources) >= 1:
            print("[CERT] LLM_CALL_START case_id=test_case")
...
def test_context_equals_citations():
    ...
    try:
        ...
        assert (
            set(context_chunk_ids) == set(cited_chunk_ids)
...
def test_context_chunks_not_modified():
    ...
    for chunk_id in original_chunks:
        assert chunk_id in cited_chunks, f"FAIL: Chunk '{chunk_id}' fue eliminado sin justificación"
