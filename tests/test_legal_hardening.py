...
if __name__ == "__main__":
    ...
    has_high_finding_001 = any(
        f.get("severity") == "high" for f in result_001.get("legal_findings", [])
    )
    ...
    has_medium_finding_002 = any(
        f.get("severity") == "medium" for f in result_002.get("legal_findings", [])
    )
    ...
    has_counter_evidence_002 = any(
        f.get("counter_evidence") for f in result_002.get("legal_findings", [])
    )
    ...
    accounting_finding = next(
        (
            f
            for f in result_003.get("legal_findings", [])
            if f.get("finding_type") == "accounting_irregularities"
        ),
        None,
    )
    ...
    assert (
        accounting_finding is not None
    ), "CASE_RETAIL_003 debe tener finding accounting_irregularities"
    ...
    assert (
        accounting_finding.get("weight") >= 100
    ), "accounting_irregularities debe tener weight >= 100"
...
def test_case_retail_003_has_accounting_irregularities():
    ...
    accounting_finding = next(
        (
            f
            for f in result.get("legal_findings", [])
            if f.get("finding_type") == "accounting_irregularities"
        ),
        None,
    )
