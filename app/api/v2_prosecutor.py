...
class ProsecutorAnalysisRequest(BaseModel):
    ...
    generate_formal_template: bool = Field(
        default=True, description="Si se debe generar la plantilla formal de acusación"
    )

    class Config:
        schema_extra = {"example": {"case_id": "CASE_RETAIL_001", "generate_formal_template": True}}
...
class ProsecutorAnalysisResponse(BaseModel):
    ...
    total_acusaciones: int = Field(..., description="Número total de acusaciones formuladas")

    plantilla_formal: Optional[PlantillaFormal] = Field(
        None, description="Plantilla formal de acusación (si se generó)"
    )

    solicitud_evidencia: Optional[dict[str, Any]] = Field(
        None, description="Solicitud de evidencia adicional (si no hay acusaciones)"
    )
        ...
        schema_extra = {
            "example": {
                ...
                "plantilla_formal": {
                    ...
                    "seccion_ii_hechos_probados": {"total_hechos": 2},
    ...
    try:
        ...
        response = ProsecutorAnalysisResponse(
            ...
            solicitud_evidencia=(
                ...
                if prosecutor_result.solicitud_evidencia
                else None
            ),
        ...
        api_requests_total.labels(
            method="POST", endpoint="/v2/prosecutor/analyze", status_code=200
        ).inc()

        api_request_duration.labels(method="POST", endpoint="/v2/prosecutor/analyze").observe(
            processing_time
        )
        ...
        # Métricas
        api_requests_total.labels(
            method="POST", endpoint="/v2/prosecutor/analyze", status_code=status_code
        ).inc()
        ...
        # Métricas
        api_requests_total.labels(
            method="POST", endpoint="/v2/prosecutor/analyze", status_code=500
        ).inc()
