...
def test_trace_context_emitted():
    ...
    try:
        ...
        assert trace_data["vectorstore_version"] == "v_20250105_120000", "FAIL: vectorstore_version incorrecto"
...
def test_decision_record_emitted():
    ...
    try:
        ...
        assert "ESTE TEXTO NO DEBE APARECER" not in full_output, \
            "FAIL: DecisionRecord contiene texto libre (PII)"
        assert "content" not in str(record_data.get("cited_chunks", [])), \
            "FAIL: DecisionRecord contiene campo 'content' con PII"
        assert "extracto_literal" not in full_output, \
...
def test_replay_uses_original_versions():
    ...
    try:
        ...
        assert replay_trace_data["replay_of"] == original_request_id, \
            "FAIL: replay_of no coincide con request_id original"
        
        # Validar que usa MISMAS versiones
        assert replay_trace_data["prompt_version"] == "v1.0.0", \
            "FAIL: Replay usa prompt_version diferente"
        assert replay_trace_data["vectorstore_version"] == "v_20250105_120000", \
            "FAIL: Replay usa vectorstore_version diferente"
        assert replay_trace_data["retrieval_top_k"] == 5, \
            "FAIL: Replay usa top_k diferente"
        
        # Validar que NO usa "latest"
        assert "latest" not in str(replay_trace_data.get("prompt_version", "")).lower(), \
            "FAIL: Replay usa 'latest' en prompt_version"
        assert "latest" not in str(replay_trace_data.get("vectorstore_version", "")).lower(), \
...
def test_no_pii_in_logs():
    ...
    assert "Este es texto sensible" not in str(sanitized), \
        "FAIL: 'content' no fue redactado"
    assert "Más texto sensible" not in str(sanitized), \
        "FAIL: 'extracto_literal' no fue redactado"
    assert "[REDACTED" in str(sanitized.get("content", "")), \
        "FAIL: 'content' no muestra [REDACTED]"
    
    # Validar que identificadores permanecen
    assert sanitized["chunk_id"] == "chunk_001", \
        "FAIL: chunk_id fue eliminado"
    assert sanitized["page"] == 1, \
        "FAIL: page fue eliminado"
