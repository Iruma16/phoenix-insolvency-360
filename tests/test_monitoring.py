...
def test_track_llm_error():
    ...
    monitor.track_llm_call(
        "auditor", case_id="CASE_001", success=False, error=Exception("LLM error")
    )
...
def test_track_rag_query():
    ...
    monitor.track_rag_query(
        "legal", case_id="CASE_001", success=True, num_results=5, duration_ms=50
    )
