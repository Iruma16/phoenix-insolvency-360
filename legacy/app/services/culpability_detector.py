...
from app.services.financial_analysis import ConfidenceLevel, Evidence
from app.services.timeline_builder import EventType, Timeline
...
def detect_asset_stripping(
    timeline: Timeline, suspect_period_start: Optional[datetime] = None
    ...
    if suspect_period_start:
        suspect_sales = [
            e for e in asset_sales if e.date >= suspect_period_start and e.is_within_suspect_period
        ]
    ...
    if len(suspect_sales) >= 2:
        ...
        risk = CulpabilityRisk(
            ...
            if all(e.confidence > 0.7 for e in suspect_sales)
...
def detect_preferential_payments(
    ...
    # Filtrar pagos
    payments = [e for e in timeline.events if e.event_type in [EventType.PAGO_REALIZADO]]

    # Pagos en periodo sospechoso
    suspect_payments = []
    if suspect_period_start:
        suspect_payments = [
            p for p in payments if p.date >= suspect_period_start and p.is_within_suspect_period
        ]
...
def detect_unjustified_outflows(timeline: Timeline) -> list[CulpabilityRisk]:
    """
...
def detect_accounting_irregularities(
    validation_result: Optional[dict], timeline: Timeline
) -> list[CulpabilityRisk]:
    """
