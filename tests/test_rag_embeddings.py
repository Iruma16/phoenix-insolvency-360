...
def test_rag_embeddings():
    ...
    try:
        ...
        expected_dim = 3072 if "3-large" in EMBEDDING_MODEL else 1536 if "3-small" in EMBEDDING_MODEL else None
        ...
        if embeddings_count > 0:
            ...
            if sample_result.get("embeddings") and len(sample_result["embeddings"]) > 0:
                ...
                print(f"ℹ️  No se pudieron obtener las dimensiones directamente de ChromaDB")
        ...
        assert result.status in ["OK", "PARTIAL_CONTEXT", "NO_RELEVANT_CONTEXT"], \
        ...
        # Generar respuesta con LLM para verificar que funciona
        if result.context_text:
            answer = build_llm_answer(
                question=question,
                context_text=result.context_text,
            )
            assert len(answer) > 0, "❌ ERROR: La respuesta generada está vacía"
            print(f"   Respuesta generada (primeros 100 chars): {answer[:100]}...")
        ...
        print(f"✅ Verificación crítica: No hubo errores de dimensiones de embeddings")
        print(f"   (El error 'Collection expecting embedding with dimension of X, got Y' no ocurrió)")
        
        print("\n" + "=" * 60)
        print("✅ TEST COMPLETADO EXITOSAMENTE")
        print("=" * 60)
        print(f"\nCaso de prueba creado: {case_id}")
        print(f"Para limpiar, puedes eliminar:")
