...
def get_case_collection(case_id: str, version: Optional[str] = None):
    ...
    if version is None:
        ...
        if not version_path.exists():
            raise RuntimeError(f"No existe versión {version} para case_id={case_id}")
...
def build_embeddings_for_case(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            f"[EMBEDDINGS] ✅ Housekeeping completado: {deleted_count} versiones eliminadas"
        )
        ...
        # --------------------------------------------------
        logger.info("=" * 60)
        logger.info("[EMBEDDINGS] ✅ Pipeline completado exitosamente")
        logger.info(f"[EMBEDDINGS] case_id: {case_id}")
        ...
        try:
            write_status(case_id, version_id, "FAILED")
            logger.error(
                f"[EMBEDDINGS] ❌ Pipeline falló. Versión marcada como FAILED: {version_id}"
            )
