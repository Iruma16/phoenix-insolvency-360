...
class DocumentSummary(BaseModel):
    ...
    document_id: str = Field(..., description="Identificador único del documento", min_length=1)

    case_id: str = Field(..., description="ID del caso al que pertenece el documento", min_length=1)

    filename: str = Field(..., description="Nombre del archivo original", min_length=1)

    file_type: str = Field(..., description="Tipo de archivo (PDF, DOCX, TXT, etc)", min_length=1)

    status: DocumentStatus = Field(
        ..., description="Estado de procesamiento calculado desde el core"
    )

    chunks_count: int = Field(default=0, description="Número real de chunks generados", ge=0)

    error_message: Optional[str] = Field(None, description="Mensaje de error si status=FAILED")

    created_at: datetime = Field(..., description="Timestamp de creación del documento")
    ...
    is_duplicate: bool = Field(
        default=False, description="Indica si este documento es un duplicado de otro"
    )

    duplicate_of_document_id: Optional[str] = Field(
        None, description="ID del documento original del que este es duplicado"
    )

    duplicate_of_filename: Optional[str] = Field(
        None, description="Nombre del archivo original del que este es duplicado"
    )

    duplicate_similarity: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Score de similaridad con el documento original (0.0-1.0)"
    )
