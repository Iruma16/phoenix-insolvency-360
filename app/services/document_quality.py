...
def get_document_quality_summary(db: Session, case_id: str) -> Dict[str, Any]:
    ...
    if docs_without_chunks > 0:
        issues.append(f"{docs_without_chunks} documento(s) sin procesar (no disponible para búsqueda semántica)")
        legal_risks.append(f"Información legal no indexada: {docs_without_chunks} documento(s) no disponible(s) para consulta")
    ...
    if total_embeddings < total_chunks:
        ...
        legal_risks.append(f"Riesgo de omisión en análisis: {missing_embeddings} fragmento(s) no indexado(s)")
    ...
    critical_docs = (
        ...
        .filter(
            ...
            Document.doc_type.in_(CRITICAL_DOCUMENT_TYPES)
        )
        .all()
    )
    ...
    if doc_legacy_count > 0:
        issues.append(f"{doc_legacy_count} archivo(s) .doc (formato legacy, puede requerir conversión)")
        legal_risks.append(f"Riesgo de pérdida de información: {doc_legacy_count} documento(s) en formato legacy")
    ...
    return {
        ...
        "critical_documents_missing": len(critical_docs_without_chunks) + len(critical_docs_without_embeddings),
    }
