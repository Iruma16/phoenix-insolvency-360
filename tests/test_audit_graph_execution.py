...
def test_audit_graph_executes_with_fixture():
    ...
    assert result["case_id"] == "CASE_RETAIL_001", f"El case_id esperado es 'CASE_RETAIL_001', pero se obtuvo '{result['case_id']}'"
...
def test_audit_graph_preserves_documents():
    ...
    assert len(result["documents"]) == 4, f"Se esperaban 4 documentos, se obtuvieron {len(result['documents'])}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   ✅ Resultado no es None")
        
        assert "case_id" in result
        print(f"   ✅ Contiene clave 'case_id'")
