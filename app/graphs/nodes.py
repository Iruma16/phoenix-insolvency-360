...
def analyze_timeline(state: AuditState) -> AuditState:
    ...
    for doc in documents:
        ...
        if description:
            events.append({"date": date, "description": description, "source_doc_id": doc_id})
    ...
    for doc in documents:
        ...
        if "insolvencia" in content_lower:
            ...
            if any(
                ...
                for phrase in ["insolvencia grave", "desde al menos", "imposibilidad de atender"]
            ):
        ...
        if "pérdida" in content_lower or "perdida" in content_lower:
            ...
            if any(
                ...
                for phrase in [
                    ...
                    "pérdidas acumuladas",
                    "patrimonio neto negativo",
                    "fondos propios inexistentes",
                ]
            ):
                has_severe_losses = True

        if any(
            word in content_lower for word in ["controlada", "viable", "viabilidad", "recuperación"]
        ):
            ...
            if any(
                phrase in content_lower
                for phrase in ["empresa es viable", "rechaza propuesta de disolución"]
            ):
...
def _detect_documentation_gap(documents: list) -> dict:
    ...
    for doc in documents:
        ...
        if any(
            phrase in content_lower
            for phrase in [
                "no se han depositado cuentas",
                "no consta contabilidad",
                "no depositado",
            ]
        ):
...
def build_report(state: AuditState) -> AuditState:
    ...
    Genera un reporte estructurado con SÍNTESIS JURÍDICA y recomendaciones accionables.
    """
    ...
    # SÍNTESIS JURÍDICA - Transforma hallazgos técnicos en criterio legal
    print("🧠 Ejecutando síntesis jurídica...")
    synthesis = synthesize_legal_position(
        risks=risks, legal_findings=legal_findings, timeline=timeline, documents=documents
    )

    # Extraer risk_level de la tesis (puede ser enum, convertir a string)
    thesis_risk = synthesis["thesis"]["risk_level"]
    if hasattr(thesis_risk, "value"):
        overall_risk = thesis_risk.value.lower()
    else:
        overall_risk = str(thesis_risk).lower()
    ...
    report = {
        "case_id": case_id,
        "synthesis": synthesis,  # ← Nueva capa de síntesis jurídica
    ...
    print(f"✅ Síntesis completada: {synthesis['thesis']['statement'][:80]}...")
...
def _calculate_overall_risk(risks: list, legal_findings: list = None) -> str:
    ...
    finding_severities = [
        finding.get("severity") for finding in legal_findings if finding.get("severity")
    ]
...
def _build_timeline_summary(timeline: list) -> str:
    ...
    summary_parts = [f"Timeline de {num_events} eventos desde {first_date} hasta {last_date}."]
    ...
    for event in timeline:
        ...
        if any(
            ...
            for keyword in ["insolvencia", "pérdida", "deterioro", "negociación"]
        ):
...
def _build_accounting_irregularities_finding(risk: dict, documents: list, evidence: list) -> dict:
    ...
    for doc in documents:
        ...
        if "auditoría externa" in content_lower or "informe pericial" in content_lower:
            ...
            mitigation.append(
                "Existe auditoría o informe pericial que puede validar la contabilidad"
            )
...
def _build_documentation_gap_finding(risk: dict, documents: list, evidence: list) -> dict:
    ...
    for doc in documents:
        content_lower = doc.get("content", "").lower()
        if (
            "no consta" in content_lower
            or "no depositadas" in content_lower
            or "no se han depositado" in content_lower
        ):
...
def _build_document_inconsistency_finding(risk: dict, documents: list, evidence: list) -> dict:
    ...
    for doc in documents:
        ...
        if "balance" in doc_type:
            if (
                "patrimonio neto negativo" in content_lower
                or "fondos propios inexistentes" in content_lower
            ):
