...
from .schema import AcusacionProbatoria, ProsecutorResult
...
def generar_seccion_antecedentes(
    ...
    documentos_db = db.query(Document).filter(Document.case_id == case_id).all()
...
def generar_plantilla_formal(
    ...
    try:
        ...
        print(
            f"[CERT] FORMAL_TEMPLATE_GENERATED case_id={prosecutor_result.case_id} "
...
def exportar_plantilla_a_texto(plantilla: PlantillaFormalAcusacion) -> str:
    ...
    lineas.append(
        f"Fecha: {plantilla.seccion_i_antecedentes.fecha_generacion.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    lineas.append("-" * 80)
    lineas.append(
        f"Total documentos analizados: {plantilla.seccion_i_antecedentes.total_documentos}"
    )
    lineas.append(
        f"Total páginas analizadas: {plantilla.seccion_i_antecedentes.total_paginas_analizadas}"
    )
    lineas.append("")

    if plantilla.seccion_i_antecedentes.observaciones_preliminares:
        lineas.append(
            f"Observaciones: {plantilla.seccion_i_antecedentes.observaciones_preliminares}"
        )
        lineas.append("")
    ...
    lineas.append(
        f"Distribución severidad: {plantilla.seccion_iii_riesgos_detectados.distribucion_severidad}"
    )
    lineas.append("")
    lineas.append(
        f"CALIFICACIÓN SUGERIDA: {plantilla.seccion_iii_riesgos_detectados.calificacion_concursal_sugerida}"
    )
