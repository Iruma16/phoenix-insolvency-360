...
def _build_alert_evidence(chunk: DocumentChunk, db: Session) -> AlertEvidence:
    ...
    if not document:
        raise ValueError(
            f"Documento {chunk.document_id} no encontrado para chunk {chunk.chunk_id}"
        )
...
def _detect_suspicious_patterns(
    ...
    very_long_chunks = [
        chunk for chunk in chunks
        if chunk.content and len(chunk.content) > 10000
    ]
    ...
    very_short_chunks = [
        chunk for chunk in chunks
        if chunk.content and 0 < len(chunk.content.strip()) < 10
    ]
