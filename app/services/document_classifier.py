...
def classify_document(
    ...
    if is_scanned:
        return DocumentClassification(
            document_type="IMAGE_SCANNED", confidence=1.0, reason="PDF escaneado o imagen detectada"
        )

    # 2. Email
    if file_extension.lower() in [".eml", ".msg"]:
        return DocumentClassification(
            document_type="EMAIL", confidence=1.0, reason=f"Extensión de email: {file_extension}"
        )
    ...
    # 6. Genérico (fallback)
    return DocumentClassification(
        document_type="GENERIC", confidence=0.3, reason="No match con patrones conocidos"
    )
