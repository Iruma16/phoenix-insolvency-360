...
def build_evidence_from_chunk(chunk: DocumentChunk) -> DocumentalEvidence:
    ...
    if chunk.start_char is None:
        raise LegalOutputError(reason=f"Chunk {chunk.chunk_id} sin start_char")

    if chunk.end_char is None:
        raise LegalOutputError(reason=f"Chunk {chunk.chunk_id} sin end_char")

    if not chunk.extraction_method:
        raise LegalOutputError(reason=f"Chunk {chunk.chunk_id} sin extraction_method")
    ...
    try:
        ...
        raise LegalOutputError(reason=f"Location inválida en chunk {chunk.chunk_id}: {e}")
...
def build_legal_report(
    ...
    if not findings_data or len(findings_data) == 0:
        raise LegalOutputError(reason="No se pueden generar findings sin datos")
    ...
    for idx, finding_data in enumerate(findings_data):
        ...
        if not statement or len(statement.strip()) < 10:
            raise LegalOutputError(reason=f"Finding {idx+1} con statement inválido o muy corto")

        # Validar que hay chunk_ids
        if not evidence_chunk_ids or len(evidence_chunk_ids) == 0:
            raise LegalOutputError(reason=f"Finding {idx+1} sin evidencia (chunk_ids vacío)")
...
def validate_legal_report(report: LegalReport) -> None:
    ...
    if not report.findings or len(report.findings) == 0:
        raise LegalOutputError(reason="Reporte sin findings")
    ...
    for idx, finding in enumerate(report.findings):
        ...
        if not finding.statement or len(finding.statement.strip()) < 10:
            raise LegalOutputError(reason=f"Finding {idx+1} con statement inválido")

        # Validar que hay evidencia
        if not finding.evidence or len(finding.evidence) == 0:
            raise LegalOutputError(reason=f"Finding {idx+1} sin evidencia")
        ...
        for ev_idx, evidence in enumerate(finding.evidence):
            ...
            if not evidence.chunk_id or not evidence.document_id:
                raise LegalOutputError(reason=f"Finding {idx+1}, evidencia {ev_idx+1} sin IDs")

            # Validar location
            if not evidence.location:
                raise LegalOutputError(reason=f"Finding {idx+1}, evidencia {ev_idx+1} sin location")
            ...
            if not evidence.content or len(evidence.content.strip()) == 0:
                raise LegalOutputError(reason=f"Finding {idx+1}, evidencia {ev_idx+1} sin content")
