...
def _build_chunk_summary(chunk: DocumentChunk, db: Session) -> ChunkSummary:
    ...
    if not chunk.extraction_method:
        raise ChunkContractViolationError(
            rule_violated=f"Chunk sin extraction_method: {chunk.chunk_id}", chunk_id=chunk.chunk_id
        )

    if not chunk.content or not chunk.content.strip():
        raise ChunkContractViolationError(
            rule_violated=f"Chunk con contenido vacío: {chunk.chunk_id}", chunk_id=chunk.chunk_id
        )
...
def list_chunks(
    ...
    text_contains: Optional[str] = Query(
        None, description="Búsqueda LITERAL en el contenido (case-insensitive)", min_length=3
    ),
    ...
    query = db.query(DocumentChunk).filter(DocumentChunk.case_id == case_id)
    ...
    if filename:
        query = query.join(Document).filter(Document.filename.ilike(f"%{filename}%"))

    # Búsqueda LITERAL en contenido (NO semántica)
    if text_contains:
        query = query.filter(DocumentChunk.content.ilike(f"%{text_contains}%"))

    # Ordenar y limitar
    chunks = query.order_by(DocumentChunk.created_at.desc()).limit(limit).all()
    ...
    for chunk in chunks:
        ...
        except ChunkContractViolationError:
