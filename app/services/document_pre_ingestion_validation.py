...
def check_min_text_length(
    text: str, min_length: int = MIN_TEXT_LENGTH_HARD
...
def check_text_noise_ratio(
    text: str, min_ratio: float = MIN_TEXT_NOISE_RATIO
...
def check_not_scanned_without_ocr(
    file_path: Path,
) -> tuple[bool, Optional[PreIngestionRejectCode], str, Optional[bool]]:
    """
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            return (
                True,
                None,
                f"PDF con texto extraíble: ~{avg_text_per_page:.0f} chars/página",
                False,
            )

    except Exception as e:
        return (False, PreIngestionRejectCode.PDF_CORRUPTED, f"Error analizando PDF: {e}", None)
...
def check_encoding_valid(
    file_path: Path,
) -> tuple[bool, Optional[PreIngestionRejectCode], str, Optional[str]]:
    """
    ...
    try:
        ...
        if not raw_data:
            return (False, PreIngestionRejectCode.FILE_EMPTY, "Archivo TXT vacío", None)
        ...
        return (
            True,
            None,
            f"Encoding válido detectado: {detected_encoding} (confianza {confidence:.2%})",
            detected_encoding,
        )
...
def check_file_size(
    file_path: Path, max_size_mb: int = MAX_FILE_SIZE_MB
) -> tuple[bool, Optional[PreIngestionRejectCode], str]:
    """
    ...
    if file_size_bytes == 0:
        return (False, PreIngestionRejectCode.FILE_EMPTY, "Archivo vacío (0 bytes)")
...
def log_pre_ingestion_validation(
    ...
    """
    logger.info("=" * 80)
    logger.info("[PRE-INGESTION VALIDATION] Resultado")
